...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if edit == false {
            ...
            next.string2 = itemList[indexPath.row].mode
            ...
            nextEdit.editlabel2 =  itemList[indexPath.row].mode
  ...
  navigationController?.pushViewController(nextEdit, animated: true)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = item.title + "   " + item.mode
        cell?.detailTextLabel?.text =  "Every " + Helper().deteToString(date: item.date)
        return cell!
    }
